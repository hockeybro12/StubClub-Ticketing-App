...
@interface RequestsStreamScreen ()

@end

@implementation RequestsStreamScreen
...
-(void)newTicket {
    ...
    [self performSegueWithIdentifier:@"NewTicketNow" sender:nil];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"NewTicketNow"]) {
...
-(void)viewDidAppear:(BOOL)animated {
    if (![[API sharedInstance] isAuthorized]) {
        ...
        if (savedPassword == nil || savedUsername == nil) {
            [self performSegueWithIdentifier:@"ShowLoginASAP" sender:nil];
...
-(void)refreshTableView {
    if ([[API sharedInstance] isAuthorized]) {
        [[API sharedInstance] commandWithParams:[NSMutableDictionary dictionaryWithObjectsAndKeys:@"streamRequests", @"command", self.navigationItem.title, @"EventName", nil] onCompletion:^(NSDictionary *json){
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1) {
        ...
        NSString *ticketID = [data objectForKey:@"RequestID"];
        
        NSLog(@"%@ %@", session, ticketID);
        
        if ([[API sharedInstance] isAuthorized]) {
            [[API sharedInstance] commandWithParams:[NSMutableDictionary dictionaryWithObjectsAndKeys:@"purchaseRequests", @"command", session, @"IdUser", ticketID, @"TicketID", nil] onCompletion:^(NSDictionary *json){
                if (![json objectForKey:@"error"]) {
